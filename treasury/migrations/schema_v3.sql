-- SYM Token D1 Schema v3 — multi-connectome trading collective + on-chain betting
...
CREATE TABLE IF NOT EXISTS user_vault_stakes (
  ...
  connectome_id TEXT NOT NULL,
  amount REAL NOT NULL,            -- SYM tokens staked
  stake_tx_hash TEXT,
...
CREATE TABLE IF NOT EXISTS user_prediction_bets (
  ...
  amount REAL NOT NULL,            -- SYM tokens wagered
  bet_tx_hash TEXT,
  claimed BOOLEAN DEFAULT 0,
  payout REAL,                     -- SYM tokens received if won
  claim_tx_hash TEXT,
...
CREATE TABLE IF NOT EXISTS prediction_rounds (
  ...
  total_pool REAL DEFAULT 0.0,     -- total SYM wagered
...
CREATE TABLE IF NOT EXISTS user_copy_trades (
  ...
  amount REAL NOT NULL,            -- SYM allocated to copy
  start_tx_hash TEXT,
  stop_tx_hash TEXT,
  realized_pnl REAL,
