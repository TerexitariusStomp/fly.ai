...
/// @notice Standing buyback bid for SYM at NAV × (1 - spread), all received SYM burned
/// @dev Direct buyback contract — users sell SYM and receive AZUSD at a discount to NAV.
///      Received SYM are burned via ERC20Burnable.burn().
///      Epoch capacity limits protect treasury outflows.
///      Circuit breaker integration: sells halt when Bucky depegs (SymbientCircuitBreaker).
///      All admin operations via multisig (Fortress/Minotaur lesson).
contract SymbientInverseBond is MultisigGuard {
    ...
    event NavUpdated(uint256 indexed _navPerSymbient, uint256 indexed _liquidTreasuryValue);
    ...
    function updateNav(uint256 _navPerSymbient, uint256 _liquidTreasuryValue) external onlyMultisig {
        navPerSymbient = _navPerSymbient;
        liquidTreasuryValue = _liquidTreasuryValue;
        emit NavUpdated(_navPerSymbient, _liquidTreasuryValue);
    }
    ...
    function bondPrice() public view returns (uint256) {
        return (navPerSymbient * (BASIS_POINTS - INVERSE_SPREAD_BPS)) / BASIS_POINTS;
    }
    ...
    function sell(uint256 shitAmount) external returns (uint256 payout) {
        ...
        // slither-disable-next-line arbitrary-send-erc20: treasury is a fixed address set by multisig, not arbitrary. This is the core inverse bond mechanism — seller burns SYM, receives AZUSD from treasury. Capacity limits protect outflows.
