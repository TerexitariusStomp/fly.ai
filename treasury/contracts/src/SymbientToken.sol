...
/// @title SymbientToken
/// @notice Main protocol token for SYM Protocol (SYM) — SYM Protocol fork pattern
...
contract SymbientToken is ERC20Permit, ERC20Burnable, MultisigGuard {
    ...
    /// @notice Authorized minter (SYM Protocol MINTR module) — can mint without multisig
    address public authorizedMinter;

    event AuthorizedMinterSet(address indexed minter);

    constructor(address _multisig) ERC20("SYM Protocol", "SYM") ERC20Permit("SYM Protocol") MultisigGuard(_multisig) {
    ...
    /// @notice Set authorized minter (SYM Protocol MINTR module) — only multisig
