...
/// @title DeploySymbientPhase2
/// @notice Phase 2 - deploy SYM Protocol V3 fork, staking, oracle, DSS stablecoin, and system safety.
/// @dev Requires Phase 1 (DeploySymbientToken) to be complete. Reads deployed token addresses from env.
///      On Base mainnet (chain ID 8453), requires non-zero UNISWAP_V3_SHIT_POOL and BOND_AGGREGATOR_ADDRESS.
contract DeploySymbientPhase2 is Script {
    ...
    function run() external {
        ...
        console2.log("=== Phase 2: SYM Protocol V3 Modules ===");

        // 15. Install SYM Protocol modules into Kernel
        ...
        // 16. SymbientPrice (fork of SYM ProtocolPrice with TWAP)
        ...
        console2.log("=== Phase 2: SYM Protocol V3 Policies ===");

        // 18. SymbientDistributor (bridges Heart -> SymbientStaking.rebase)
        ...
        // 22. SymbientDefenseBudget - per-epoch treasury spending cap wrapping Operator.operate()
        ...
        // 24. Set authorized minter to SYM ProtocolMinter
        if (safe == msg.sender) {
            SymbientToken(shitTokenAddr).setAuthorizedMinter(address(minter));
        } else {
        ...
        console2.log("  5. Initialize SymbientPrice with start observations via Heart beat");
