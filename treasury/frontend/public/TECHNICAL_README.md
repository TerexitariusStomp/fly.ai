# SYM Protocol — Technical Documentation

Full-stack DeFi protocol for environmental impact tokens. SYM Protocol combines bonding, protocol-owned liquidity (POL), a stablecoin (Bucky), governance, yield-bearing vaults, and perpetual trading into a single integrated system built on Ethereum.
...
SYM Protocol is a DeFi protocol on Base that enables:

1. **Token issuance** — SYM ERC20 with 100M max supply and multisig-gated minting.
2. **Staking** — Rebasing stSYM (8-hour epochs, POL yield + supplemental emissions, circuit-breaker protected) with a non-rebasing wstSYM wrapper (Lido wstETH pattern) for DeFi composability.
3. **Bonding** — Inverse bonds: a standing buyback bid at NAV × (1 − spread) that burns received SYM. Circuit-breaker protected, adjustable capacity. Standard discount bonds are sold via Bond Protocol's own infrastructure, not custom contracts.
...
7. **NAV Warmup Ramp** — Supplemental mint amount in `SymbientStaking` ramps up gradually during the initial epochs after launch to prevent sudden inflation at low supply.

Governance is off-chain via a Safe multisig (`MultisigGuard`) — there is no on-chain governance contract. Larger DeFi primitives referenced by the frontend (vaults, lending, perps, index tokens, yield trading, POL across multiple venues) are accessed by integrating directly with external protocols (Yearn, Morpho, GMX, Pendle, etc.) rather than through custom SYM Protocol contracts — see the `// no custom ... contracts` comments in `contracts/script/DeployAll.s.sol` for the exact split.
...
│   ├── script/         # Foundry deployment scripts (DeployAll, DeploySymbientFork)
...
The protocol uses the **SYM Protocol V3 Kernel & Policy** pattern:
...
2. **RFV/NAV Tracking** — `TreasuryValuation` computes NAV and risk-free value (RFV) from registered treasury assets (with per-asset haircuts) and gates `SymbientStaking.rebase()` supplemental emissions.
3. **Fail-Closed Oracles** — `SymbientPriceFeed`/`ImpactOracleAdapter` revert on stale or insufficient TWAP observations rather than returning a stale price.
...
- **`SymbientToken.sol`** — ERC20 with permit, burnable, multisig-gated minting, 100M max supply
...
### SYM Protocol V3 Forks (Staking, Treasury, Bonds)

- **`SymbientStaking.sol`** — Rebasing stSYM token, 8-hour epochs, POL yield + supplemental emissions
- **`wstSYM.sol`** — Non-rebasing wrapper (Lido wstETH pattern)
- **`StakingAdapter.sol`** — Bridges SymbientStaking to SYM Protocol V3 IStaking interface
- **`TreasuryValuation.sol`** — RFV/NAV tracking via SYM Protocol TRSRYv1, multisig-gated
- **`ITreasuryPolicy.sol`** / **`IValuationCalculator.sol`** — Interfaces for treasury valuation
- **`SymbientPrice.sol`** — Fork of SYM ProtocolPrice using TWAP instead of Chainlink
- **`SymbientDistributor.sol`** — Bridges SYM Protocol Heart beat to SymbientStaking rebase
- **`SymbientDefenseBudget.sol`** — Per-epoch treasury spending cap wrapping Operator.operate(). Circuit-breaker protected (halts when CB is tripped).
- **`SymbientBondPricer.sol`** — Dynamic bond discount tied to treasury/RFV growth ratio
- **`SymbientInverseBond.sol`** — Standing buyback bid at NAV x (1 - spread), burns received SYM. Circuit-breaker protected (halts when CB is tripped). Adjustable capacity via `maxCapacityBps`.

### Oracle

- **`SymbientPriceFeed.sol`** — TWAP price feed from Uniswap V3 pool, implements IPriceFeed + ITwapPriceFeed
...
- **`SymbientCollateralManager.sol`** — Registers collateral types (ilks) in DSS, deploys cloneable GemJoin/Clipper/OSM
...
- **`SymbientLiquidationKeeper.sol`** — Flash-loan liquidation keeper (DSS ClipperCallee pattern)
- **`SymbientTreasuryIntegration.sol`** — OZ TimelockController for treasury spell execution
...
- **`SymbientCircuitBreaker.sol`** — Global Bucky depeg circuit breaker (21-epoch recovery period to prevent whipsaw re-enabling during stress)
...
- **`SymbientDeployer.sol`** — CREATE2 deterministic deployment with vanity salts
...
- **`DeploySymbientFork.s.sol`** — SYM Protocol V3 fork components only (Kernel, modules, policies)
...
- **SYM Protocol V3** — Kernel, Policy, Module framework, Operator, Heart, Range, ROLES, MINTR, TRSRY
...
|---------------|-----------|---------|-------|
| SYM Protocol v2 (primary base) | `https://github.com/SYM Protocol/symbient-protocol-frontend-v2` | MIT | App shell, layout, design system, staking/governance/treasury modules |
...
| SYM Protocol V3 | `https://github.com/SYM Protocol/symbient-protocol-v3` | Kernel, treasury, policies |
...
- **Modules**: Feature pages/hooks/libs live in `modules/` (flat). Each feature is prefixed (e.g., `izipay-izipay-dashboard-page.tsx`, `protocol-protocol-overview-page.tsx`, `symbient-balance-page.tsx`).
...
| `VITE_SHIT_UNITS_API_ENDPOINT` | Yes | SYM Protocol Units API base URL |
...
- **`connect-button.tsx`** — RainbowKit wallet connection button with custom SYM Protocol styling.
...
- **`symbient-logo.tsx`** — SYM Protocol SVG logo.
...
- **`wrap-symbient-modal.tsx`** — Wrap SYM → stSYM.
- **`unstake-stsym-modal.tsx`** — Unstake stSYM → SYM.
- **`unwrap-wstsym-modal.tsx`** — Unwrap wstSYM → stSYM.
- **`migrate-symbient-modal.tsx`** — V1 → V2 migration with Merkle proof.
- **`mint-testnet-symbient-modal.tsx`** / **`mint-testnet-usds-modal.tsx`** — Testnet token minting (testnet only).
...
- **`tokens.ts`**: Token definitions (SYM, stSYM, wstSYM, USDS, etc.)
...
- **`footer.tsx`** — Sticky footer displaying real-time protocol data: Next Beat timer (countdown to next RBS beat), token prices (SYM, BUCKY), gas tracker, and theme toggle button.
...
- **`symbient-*`** — SYM token management — balance dashboard (all SYM variants), wrap/stake flows, V1 migration with Merkle proof.
...
- **`pulse-*`** — Price history hooks — SYM and wstSYM price history data.
...
- **`protocol-protocol-overview-page.tsx`** — Main landing page (~850 lines). Composes all protocol sections into a single scrolling page. Imports and renders module pages from `symbient-*`, `bonds-*`, `rbs-*`, `cooler-*`, and protocol widget components. Key sections: Overview (`id="overview"`), Tokens (`id="tokens"`), Dashboard (`id="dashboard"`), Markets (`id="markets"`), Treasury (`id="treasury"`).
...
- **`protocol-widget-contract-widgets.tsx`** — Exports `TreasuryMetricsWidget` (RFV, NAV, floor price, NAV per SYM), `PriceFeedWidget` (current SYM price, TWAP), `POLManagementWidget` (POL positions with deposit/withdraw/harvest).
- **`protocol-widget-floor-market-widget.tsx`** — Floor market dashboard — floor price, redemption availability, USDC backing status.
- **`protocol-widget-faucet-widget.tsx`** — Testnet faucet widget (testnet only) — mint testnet SYM/USDS.
...
Use `DeploySymbientFork.s.sol` to deploy only SYM Protocol V3 fork components (assumes core tokens already deployed).
...
The entire SYM Protocol codebase is licensed under **AGPL-3.0-only**. See [LICENSE](LICENSE) for the full text.
