...
class Day():
    def __init__(self, params:ModelParams, prev_arbs:Dict[int, Tuple[float, float]], prev_lags=Dict[int, Tuple[int, Dict[int, float]]], prev_day=None, historical_net_flows=None):
        ...
        prev_lags['gsym price variation'][1][self.day] = self.price * (1 + self.reward_rate)        
        self.gsym_volatility = calc_gsym_volatility(prev_lags=prev_lags)
...
# gSYM volatility
def calc_gsym_volatility(prev_lags:Dict[int, Tuple[int, Dict[int, float]]]):
    days = len(prev_lags['gsym price variation'][1]) - 1
    data = list(prev_lags['gsym price variation'][1].values())
    if days > 6:
        if np.mean(data[-6:]) == 0:
           return 0
        else:
