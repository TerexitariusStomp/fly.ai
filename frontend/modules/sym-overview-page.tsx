...
import { useApi, toTraderSummaries, type Connectome, type Governance, type TreasuryPoint, type Treasury } from "@/lib/sym-api";
...
const API_BASE = import.meta.env.VITE_SYM_UNITS_API_ENDPOINT ?? "https://api-worker.hardwoodstablecoin.workers.dev";
...
export function SymOverviewPage() {
  const [selectedId, setSelectedId] = useState<string>("drosophila");
  ...
  const { data: history } = useApi<TreasuryPoint[]>("/api/treasury/history", 30000);
  const { data: treasury } = useApi<Treasury>("/api/treasury", 30000);

  const traders = connectomes ? toTraderSummaries(connectomes, governance) : [];
  // Build chart data — treasury reserve USD + floor price
  const treasuryChart = history
    ? history.slice().reverse()
        .filter(p => p.reserve_usd > 0 || p.total_rfv > 0)
        .map(p => ({
    ...
    : [];
  const hasRfvData = treasuryChart.some(p => p.rfv !== null);
    ...
    <div className="min-h-screen bg-[#07090c] text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          <section>
            <h2 className="font-mono text-sm uppercase tracking-wider text-gray-400 mb-3">Treasury</h2>
            <div className="rounded-xl border border-gray-800 bg-[#0a0d12] p-4">
              {treasuryChart.length > 0 ? (
                <>
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <div className="font-mono text-xs uppercase text-gray-500">Treasury RFV</div>
                      <div className="font-mono text-lg text-emerald-400">${treasury?.rfv?.toFixed(4) ?? "—"}</div>
                    </div>
                    <div className="text-right">
                      <div className="font-mono text-xs uppercase text-gray-500">Floor Price</div>
                      <div className="font-mono text-lg text-gray-300">${treasury?.floor_price?.toFixed(7) ?? "0"}</div>
                    </div>
                  </div>
                  <ResponsiveContainer width="100%" height={180}>
                    <AreaChart data={treasuryChart}>
                      <defs><linearGradient id="rfvGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#6cf08a" stopOpacity={0.4} /><stop offset="100%" stopColor="#6cf08a" stopOpacity={0} /></linearGradient></defs>
                      ...
                      <Area type="monotone" dataKey="reserve" stroke="#6cf08a" strokeWidth={2} fill="url(#rfvGrad)" name="Reserve USD" />
                      {hasRfvData && (
                        <Area type="monotone" dataKey="rfv" stroke="#3ed8ff" strokeWidth={1} fill="none" name="RFV" connectNulls />
                      )}
                    </AreaChart>
                  </ResponsiveContainer>
                </>
              ) : <EmptyState text="Loading treasury data..." />}
            </div>
          </section>
          <section>
