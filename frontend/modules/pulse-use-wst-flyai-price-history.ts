...
export interface WstFlyaiPriceHistory {
  dataPoints: Array<{ date: string; price: number }>;
}

export function useWstFlyaiPriceHistory() {
  return useQuery<WstFlyaiPriceHistory>({
    queryKey: ["wstFlyaiPriceHistory", "treasury-subgraph"],
    queryFn: async () => {
