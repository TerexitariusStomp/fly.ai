...
import { WrapFlyaiModal } from "@/components/wrap-flyai-modal";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui-tabs.tsx";
import { useWstFlyaiConversion } from "@/hooks/use-wst-flyai-conversion";
...
import { useWstFlyaiConversionRate } from "@/hooks/use-wst-flyai-conversion.tsx";
import { PriceChange } from "@/components/price-change.tsx";
import { useFlyaiPriceHistory } from "@/modules/pulse-use-flyai-price-history.ts";
import { useWstFlyaiPriceHistory } from "@/modules/pulse-use-wst-flyai-price-history.ts";
...
function WrapInfoCards() {
  ...
  const { flyaiPerWstFlyai: flyaiPerWstFlyai, wstFlyaiPerFlyai: wstFlyaiPerFlyai, isLoading: indexLoading } = useWstFlyaiConversionRate();
  const { apy, isLoading: apyLoading } = useStakingAPY();
  const { data: flyaiPriceHistory } = useFlyaiPriceHistory();
  const { data: wstFlyaiPriceHistory } = useWstFlyaiPriceHistory();

  const flyaiRef24h = getReferenceSnapshot(flyaiPriceHistory?.dataPoints ?? [], 24);
  const wstFlyaiRef24h = getReferenceSnapshot(wstFlyaiPriceHistory?.dataPoints ?? [], 24);
  const flyaiChange24h =
    flyaiRef24h && FlyaiToken.price > 0
      ? ((FlyaiToken.price - flyaiRef24h.price) / flyaiRef24h.price) * 100
      : null;
  const wstFlyaiChange24h =
    wstFlyaiRef24h && WstFlyaiToken.price > 0
      ? ((WstFlyaiToken.price - wstFlyaiRef24h.price) / wstFlyaiRef24h.price) * 100
      : null;
  ...
  const toValue = Number(inverted ? wstFlyaiPerFlyai : flyaiPerWstFlyai);
      ...
      <Card className="flex flex-col gap-1 p-6">
        ...
        <div className="flex items-center gap-x-2">
          {!FlyaiToken.price ? (
            ...
            <NumberFlow className="text-[20px]/[24px] font-semibold" value={FlyaiToken.price} />
          )}
          {flyaiChange24h !== null && <PriceChange percentage={flyaiChange24h} timeframe="24h" />}
        </div>
      </Card>
      ...
      <Card className="flex flex-col gap-1 p-6">
        ...
        <div className="flex items-center gap-x-2">
          {!WstFlyaiToken.price ? (
            ...
            <NumberFlow className="text-[20px]/[24px] font-semibold" value={WstFlyaiToken.price} />
          )}
          {wstFlyaiChange24h !== null && <PriceChange percentage={wstFlyaiChange24h} timeframe="24h" />}
        </div>
      </Card>
...
function WrapForm({
  ...
  const flyaiBase = useToken(TokenName.FLYAI, address);
  const stFlyaiBase = useToken(TokenName.STFLYAI, address);
  const wstFlyaiBase = useToken(TokenName.WSTFLYAI, address);

  const tokensByName = useMemo<Record<WrapPageToken, TokenWithBalance>>(
    () => ({
      [TokenName.FLYAI]: { ...flyaiBase, price: FlyaiToken.price },
      [TokenName.STFLYAI]: { ...stFlyaiBase, price: FlyaiToken.price },
      [TokenName.WSTFLYAI]: { ...wstFlyaiBase, price: WstFlyaiToken.price },
    }),
    [flyaiBase, stFlyaiBase, wstFlyaiBase, FlyaiToken.price, WstFlyaiToken.price],
  );
...
function WrapBalancePanel({ flow, inputAmount, outputAmount }: { flow: WrapFlow; inputAmount: string; outputAmount: string }) {
  ...
  const flyaiAddress = getTokenAddress(TokenName.FLYAI, chainId ?? 0);
  const stFlyaiAddress = getTokenAddress(TokenName.STFLYAI, chainId ?? 0);
  const wstFlyaiAddress = getTokenAddress(TokenName.WSTFLYAI, chainId ?? 0);

  const { balance: flyaiBalance } = useTokenBalance(flyaiAddress, address);
  const { balance: stFlyaiBalance } = useTokenBalance(stFlyaiAddress, address);
  const { balance: wstFlyaiBalance } = useTokenBalance(wstFlyaiAddress, address);

  const balances: Record<PanelToken, number> = {
    [TokenName.FLYAI]:
      flyaiBalance != null ? parseFloat(formatUnits(flyaiBalance, TOKENS.FLYAI.decimals)) : 0,
    [TokenName.STFLYAI]:
      stFlyaiBalance != null ? parseFloat(formatUnits(stFlyaiBalance, TOKENS.STFLYAI.decimals)) : 0,
    [TokenName.WSTFLYAI]:
      wstFlyaiBalance != null ? parseFloat(formatUnits(wstFlyaiBalance, TOKENS.WSTFLYAI.decimals)) : 0,
  };
...
export function WrapPage() {
  ...
  const { outputAmount } = useWstFlyaiConversion(WRAP_FLOWS[flow].conversion, inputAmount);
