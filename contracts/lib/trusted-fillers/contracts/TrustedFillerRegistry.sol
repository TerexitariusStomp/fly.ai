...
contract TrustedFillerRegistry is ITrustedFillerRegistry {
    ...
    constructor(address _roleRegistry) {
        if (!(_roleRegistry != address(0))) revert TrustedFillerRegistry__InvalidRoleRegistry();

        roleRegistry = IRoleRegistry(_roleRegistry);
    }

    function addTrustedFiller(IBaseTrustedFiller _filler) external {
        if (!(roleRegistry.isOwner(msg.sender))) revert TrustedFillerRegistry__InvalidCaller();
        if (!(address(_filler) != address(0))) revert TrustedFillerRegistry__InvalidFiller();
    ...
    function deprecateTrustedFiller(IBaseTrustedFiller _filler) external {
        if (!(roleRegistry.isOwnerOrEmergencyCouncil(msg.sender))) revert TrustedFillerRegistry__InvalidCaller();
        if (!(address(_filler) != address(0))) revert TrustedFillerRegistry__InvalidFiller();
    ...
    {
        if (!(trustedFillers[trustedFiller])) revert TrustedFillerRegistry__InvalidFiller();
