...
library RebalancingLib {
    ...
    function startRebalance(
        ...
        if (!(block.timestamp <= rebalanceParams.deadline)) revert IFolio.Folio__DeadlineExpired();

        uint256 nextRebalanceNonce = rebalance.nonce + 1;
        if (!(rebalanceNonce == nextRebalanceNonce)) revert IFolio.Folio__InvalidRebalanceNonce();
        ...
        )) revert IFolio.Folio__InvalidTTL();
        ...
        )) revert IFolio.Folio__InvalidLimits();

        uint256 len = tokens.length;
        if (!(len > 1)) revert IFolio.Folio__EmptyRebalance();

        // set new rebalance details and prices
        for (uint256 i; i < len; i++) {
            IFolio.TokenRebalanceParams calldata params = tokens[i];
            if (!(params.inRebalance)) revert IFolio.Folio__NotInRebalance();

            // enforce valid token
            if (!(params.token != address(0) && params.token != address(this))) revert IFolio.Folio__InvalidAsset();

            // enforce no duplicates
            if (!(!rebalance.details[params.token].inRebalance)) revert IFolio.Folio__DuplicateAsset();
        ...
        )) revert IFolio.Folio__InvalidWeights();
            } else {
        ...
        )) revert IFolio.Folio__InvalidPrices();
    ...
    function openAuction(
        ...
        uint256 len = tokens.length;
        if (!(len != 0 && len == weights.length && len == prices.length)) revert IFolio.Folio__InvalidArrayLengths();
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            // enforce unique
            if (!(auction.prices[token].high == 0)) revert IFolio.Folio__DuplicateAsset();
        ...
        )) revert IFolio.Folio__InvalidAsset();
            ...
            prices[i].low >= rebalanceDetails.initialPrices.low &&
                    ...
                    if (!(allAtomicSwaps == (prices[i].low == prices[i].high))) revert IFolio.Folio__MixedAtomicSwaps();
                }
    ...
    function getBid(
        ...
        if (!(sellAvailable >= params.minSellAmount)) revert IFolio.Folio__InsufficientSellAvailable();
        ...
        if (!(bidAmount <= params.maxBuyAmount)) revert IFolio.Folio__SlippageExceeded();
    }
    ...
    function bid(
        ...
        if (!(bidAmount != 0)) revert IFolio.Folio__InsufficientBuyAvailable();
        ...
        uint256 bought;
        {
            ...
            if (!(bought >= bidAmount)) revert IFolio.Folio__InsufficientBid();
        }
    ...
    function _price(
        ...
        )) revert IFolio.Folio__AuctionNotOngoing();
