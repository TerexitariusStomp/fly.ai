...
contract FolioDeployer is IFolioDeployer, Versioned {
    ...
    function deployFolio(
        ...
        if (!(basicDetails.assets.length == basicDetails.amounts.length)) revert FolioDeployer__LengthMismatch();
    ...
    function deployGovernedFolio(
        ...
        if (!(stToken != address(0))) revert FolioDeployer__InvalidStToken();
