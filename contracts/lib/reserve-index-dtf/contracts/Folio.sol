    ...
    Versioned
{
    ...
    modifier notDeprecated() {
        if (!(!isDeprecated)) revert Folio__FolioDeprecated();
        _;
    }
    ...
    function initialize(
        ...
        if (!(_basicDetails.initialShares != 0)) revert Folio__ZeroInitialShares();

        uint256 assetLength = _basicDetails.assets.length;
        if (!(assetLength != 0)) revert Folio__EmptyAssets();

        for (uint256 i; i < assetLength; i++) {
            if (!(_basicDetails.assets[i] != address(0))) revert Folio__InvalidAsset();

            uint256 assetBalance = IERC20(_basicDetails.assets[i]).balanceOf(address(this));
            if (!(assetBalance != 0)) revert Folio__InvalidAssetAmount(_basicDetails.assets[i]);
    ...
    function redeem(
        ...
        if (!(len == assets.length && len == minAmountsOut.length)) revert Folio__InvalidArrayLengths();

        bool toSelf = receiver == address(this);

        for (uint256 i; i < len; i++) {
            if (!(_assets[i] == assets[i])) revert Folio__InvalidAsset();
            if (!(_amounts[i] >= minAmountsOut[i])) revert Folio__InvalidAssetAmount(_assets[i]);
    ...
    function getAuctionPrice(uint256 auctionId, address token) external view returns (PriceRange memory range) {
        ...
        if (!(range.low != 0)) revert Folio__InvalidAsset();
    }
    ...
    function startRebalance(
        ...
        if (tradeAllowlistEnabled) {
            for (uint256 i; i < tokens.length; i++) {
                if (!(tradeTokenAllowlist.contains(tokens[i].token))) revert Folio__TokenNotAllowlisted();
            }
        }
    ...
    function openAuction(
        ...
        if (rebalance.priceControl == PriceControl.NONE) {
            if (!(auctionLength == maxAuctionLength)) revert Folio__InvalidAuctionLength();
        } else {
            if (!(
        ...
        uint256 len = tokens.length;
        for (uint256 i; i < len; i++) {
            if (!(rebalance.details[tokens[i]].inRebalance)) revert Folio__InvalidAsset();
        }
    ...
    function openAuctionUnrestricted(
        ...
        if (!(block.timestamp >= rebalance.restrictedUntil)) revert Folio__AuctionCannotBeOpenedWithoutRestriction();
    ...
    function bid(
        ...
        if (!(rebalance.bidsEnabled)) revert Folio__PermissionlessBidsDisabled();
    ...
    function createTrustedFill(
        ...
        )) revert Folio__TrustedFillerRegistryNotEnabled();
        ...
        if (!(buyAmount != 0)) revert Folio__InsufficientBuyAvailable();
    ...
    function emergencyCloseTrustedFill(address trustedFill) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        ...
        )) revert Folio__InvalidTrustedFill();
    ...
    function _openAuction(
        ...
        if (auctionId != 0) {
            ...
            if (
                ...
                if (!(auctionBuffer == 0)) revert Folio__AuctionCannotBeOpenedWithoutRestriction();
    ...
    function _setMintFee(uint256 _newFee) internal {
        if (!(_newFee <= MAX_MINT_FEE)) revert Folio__MintFeeTooHigh();
    ...
    function _setFolioSelfFee(uint256 _newFee) internal {
        if (!(_newFee <= MAX_FOLIO_FEE)) revert Folio__FolioFeeTooHigh();
    ...
    function _setMaxAuctionLength(uint256 _newLength) internal {
        if (!(_newLength >= MIN_AUCTION_LENGTH && _newLength <= MAX_AUCTION_LENGTH)) revert Folio__InvalidAuctionLength();
    ...
    function _addToBasket(address token) internal {
        if (!(token != address(0) && token != address(this))) revert Folio__InvalidAsset();
    ...
    function _setTrustedFillerRegistry(address _newFillerRegistry, bool _enabled) internal {
        if (address(trustedFillerRegistry) != _newFillerRegistry) {
            if (!(address(trustedFillerRegistry) == address(0))) revert Folio__TrustedFillerRegistryAlreadySet();
    ...
    function _setDaoFeeRegistry(address _newDaoFeeRegistry) internal {
        if (!(_newDaoFeeRegistry != address(0))) revert Folio__InvalidRegistry();
    ...
    function _update(address from, address to, uint256 value) internal override {
        ...
        if (!(to != address(this))) revert Folio__InvalidTransferToSelf();
