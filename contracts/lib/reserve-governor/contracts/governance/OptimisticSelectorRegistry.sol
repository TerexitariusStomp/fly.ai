...
contract OptimisticSelectorRegistry is Initializable, IOptimisticSelectorRegistry {
    ...
    modifier onlyTimelock() {
        if (!(msg.sender == governor.timelock())) revert SelectorRegistry__OnlyOwner(msg.sender);
        _;
    }
    ...
    function _add(address target, bytes4[] memory selectors) internal {
        ...
        )) revert SelectorRegistry__InvalidTarget(target);

        for (uint256 i = 0; i < selectors.length; i++) {
            // no empty selectors
            if (!(selectors[i] != bytes4(0))) revert SelectorRegistry__InvalidSelector(selectors[i]);
