    ...
    IReserveOptimisticGovernor
{
    ...
    function isOptimistic(uint256 proposalId) external view returns (bool) {
        if (!(_proposalCore(proposalId).voteStart != 0)) revert GovernorNonexistentProposal(proposalId);
    ...
    function _queueOperations(
        ...
        if (!(!_isOptimistic(proposalId))) revert OptimisticGovernor__OptimisticProposalCannotBeQueued(proposalId);
        ...
        returns (uint256)
    {
        ...
        )) revert OptimisticGovernor__OptimisticProposalCanOnlyBeVetoed(proposalId);
    ...
    function _setProposalThrottle(uint256 newCapacity) internal {
        ...
        )) revert OptimisticGovernor__InvalidProposalThrottle();
    ...
    function _setVotingDelay(uint48 newVotingDelay) internal override {
        if (!(newVotingDelay < MAX_OPTIMISTIC_DELAY)) revert OptimisticGovernor__InvalidDelay();

        super._setVotingDelay(newVotingDelay);
    }

    function _setLateQuorumVoteExtension(uint48 newVoteExtension) internal override {
        if (!(newVoteExtension < MAX_VOTE_EXTENSION)) revert OptimisticGovernor__InvalidDelay();
    ...
    function _setOptimisticParams(OptimisticGovernanceParams calldata params) private {
        ...
        )) revert OptimisticGovernor__InvalidOptimisticParameters();
