...
library ProposalLib {
    ...
    function proposeOptimistic(
        ...
        )) revert IReserveOptimisticGovernor.OptimisticGovernor__NotOptimisticProposer(proposal.proposer);
        ...
        )) revert IReserveOptimisticGovernor.OptimisticGovernor__InvalidCall(target, proposal.calldatas[i]);
            }
        }
        ...
        external
    {
        ...
        )) revert IGovernor.GovernorInsufficientProposerVotes(proposal.proposer, proposerVotes, votesThreshold);
        }
        ...
        view
    {
        ...
        )) revert IGovernor.GovernorRestrictedProposer(proposal.proposer);

        if (!(
            bytes18(bytes(proposal.description)) != CONFIRMATION_PREFIX_BYTES
        )) revert IReserveOptimisticGovernor.OptimisticGovernor__ConfirmationPrefixNotAllowed();

        if (!(
            proposal.targets.length == proposal.values.length && proposal.targets.length == proposal.calldatas.length
        )) revert IGovernor.GovernorInvalidProposalLength(proposal.targets.length, proposal.calldatas.length, proposal.values.length);

        if (!(proposal.targets.length != 0)) revert IGovernor.GovernorInvalidProposalLength(0, 0, 0);
    }
