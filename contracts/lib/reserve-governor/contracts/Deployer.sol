...
contract ReserveOptimisticGovernorDeployer is Versioned, IReserveOptimisticGovernorDeployer {
    ...
    constructor(
        ...
        if (!(address(_versionRegistry) != address(0))) revert Deployer__InvalidVersionRegistry();
        if (!(address(_rewardTokenRegistry) != address(0))) revert Deployer__InvalidRewardTokenRegistry();
        if (!(address(_trustedFillerRegistry) != address(0))) revert Deployer__InvalidTrustedFillerRegistry();
        if (!(address(_guardian) != address(0))) revert Deployer__InvalidGuardian();

        if (!(address(_stakingVaultImpl) != address(0))) revert Deployer__InvalidStakingVaultImpl();
        if (!(address(_governorImpl) != address(0))) revert Deployer__InvalidGovernorImpl();
        if (!(address(_timelockImpl) != address(0))) revert Deployer__InvalidTimelockImpl();
        if (!(address(_selectorRegistryImpl) != address(0))) revert Deployer__InvalidSelectorRegistryImpl();
    ...
    function deployWithExistingStakingVault(
        ...
        if (!(existingStakingVault.code.length != 0)) revert Deployer__InvalidStakingVault();
    ...
    function _deployOptimisticGovernance(
        ...
        )) revert Deployer__InvalidStakingVault();
