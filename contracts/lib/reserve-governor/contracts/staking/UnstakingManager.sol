...
contract UnstakingManager {
    ...
    function createLock(address user, uint256 amount, uint256 unlockTime) external {
        if (!(msg.sender == address(vault))) revert UnstakingManager__Unauthorized();
    ...
    function cancelLock(uint256 lockId) external {
        ...
        if (!(user == msg.sender)) revert UnstakingManager__Unauthorized();
        if (!(lock.claimedAt == 0)) revert UnstakingManager__AlreadyClaimed();
    ...
    function claimLock(uint256 lockId) external {
        ...
        if (!(lock.unlockTime <= block.timestamp && lock.unlockTime != 0)) revert UnstakingManager__NotUnlockedYet();
        if (!(lock.claimedAt == 0)) revert UnstakingManager__AlreadyClaimed();
