...
CREATE TABLE IF NOT EXISTS flies (id text primary key default (lower(hex(randomblob(16)))), owner text , name text not null unique , color text not null default '#e0342c', patch_id text not null , senses text not null default '{}', temperament text not null default '{}', dials text not null default '{}', wiring_variant text not null default 'male-cns-v1.0', seed int not null default 0, created_at text not null default (datetime('now')), burned real not null default 0, burn_tx text, auto_born integer not null default 0, governor_eligible integer not null default 0, active integer not null default 1, x real, y real, heading real, elo integer not null default 1000, duels integer not null default 0, wins integer not null default 0, losses integer not null default 0, draws integer not null default 0, parents text, generation integer not null default 1);
...
CREATE TABLE IF NOT EXISTS posts (id integer primary key, tick_id bigint not null , fly_id text not null , patch_id text not null , word text not null, confidence real not null, truth text not null, correct integer generated always as (word = truth) stored, wing_hz real, neurons text not null default '[]', created_at text not null default (datetime('now')), kind text, actions text, poke_id integer, cause text, trace text);
